...
class Agent(object):
	...
	def calculate_obstacles_delta(self, x, y):
		...
		return delta_xO, delta_yO, sqnorm
	...
	def move_to_position(self, tank, target_x, target_y):
		...
		delta_xO, delta_yO, magO = self.calculate_obstacles_delta(tank.x, tank.y)
		delta_xR, delta_yR = self.calculate_random_delta()
		
		#combine
		delta_x = delta_xG + delta_xO + delta_xR
		delta_y = delta_yG + delta_yO + delta_yR
		
		print "gx: ", delta_xG, " ox: ", delta_xO , " rx: ", delta_xR
		print "gy: ", delta_yG, " oy: ", delta_yO , " ry: ", delta_yR
		...
		#put lower bound on speed: no slower than 20%
		if magnitude < 0.2:
			magnitude = 0.2
...
class GnuPlot():
	...
	def generate_field_function(self, scale):
		...
		def function(x, y):
			...
			delta_xO, delta_yO, magnitude = self.agent.calculate_obstacles_delta(x, y)
